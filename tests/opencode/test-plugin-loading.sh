#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the goodvibes plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/goodvibes.js"
...
skill_count=$(find "$GOODVIBES_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $GOODVIBES_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-goodvibes skill exists (critical for bootstrap)
echo "Test 3: Checking using-goodvibes skill (required for bootstrap)..."
if [ -f "$GOODVIBES_SKILLS_DIR/using-goodvibes/SKILL.md" ]; then
    echo "  [PASS] using-goodvibes skill exists"
else
    echo "  [FAIL] using-goodvibes skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$GOODVIBES_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/goodvibes/' "$GOODVIBES_PLUGIN_FILE"; then
