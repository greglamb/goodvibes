...
# Verifies that skills are resolved with correct priority: project > personal > goodvibes
...
# 1. Create in goodvibes location (lowest priority)
mkdir -p "$GOODVIBES_SKILLS_DIR/priority-test"
cat > "$GOODVIBES_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Goodvibes version of priority test skill
---
# Priority Test Skill (Goodvibes Version)

This is the GOODVIBES version of the priority test skill.

PRIORITY_MARKER_GOODVIBES_VERSION
EOF
...
if [ -f "$GOODVIBES_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Goodvibes version exists"
else
    echo "  [FAIL] Goodvibes version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides goodvibes
echo ""
echo "Test 2: Testing personal > goodvibes priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides goodvibes)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_GOODVIBES_VERSION"; then
    echo "  [FAIL] Goodvibes version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|goodvibes" | head -10
fi

# Test 3: Test that project overrides both personal and goodvibes
echo ""
echo "Test 3: Testing project > personal > goodvibes priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Goodvibes version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit goodvibes: prefix bypasses priority
echo ""
echo "Test 4: Testing goodvibes: prefix forces goodvibes version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load goodvibes:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_GOODVIBES_VERSION"; then
    echo "  [PASS] goodvibes: prefix correctly forces goodvibes version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] goodvibes: prefix did not force goodvibes version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
