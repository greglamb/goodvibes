...
> **For agentic workers:** REQUIRED SUB-SKILL: Use goodvibes:executing-plans to implement this plan task-by-task.

**Goal:** Add full goodvibes support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/goodvibes-codex` (lines 40-74)
...
- Reference: `.codex/goodvibes-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'goodvibes' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/goodvibes-codex` (lines 212-280)
 ...
 * (personal skills override goodvibes skills).
 *
 * @param {string} skillName - Name like "goodvibes:brainstorming" or "my-skill"
 * @param {string} goodvibesDir - Path to goodvibes skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, goodvibesDir, personalDir) {
    // Strip goodvibes: prefix if present
    const forceGoodvibes = skillName.startsWith('goodvibes:');
    const actualSkillName = forceGoodvibes ? skillName.replace(/^goodvibes:/, '') : skillName;

    // Try personal skills first (unless explicitly goodvibes:)
    if (!forceGoodvibes && personalDir) {
    ...
    // Try goodvibes skills
    if (goodvibesDir) {
        const goodvibesPath = path.join(goodvibesDir, actualSkillName);
        const goodvibesSkillFile = path.join(goodvibesPath, 'SKILL.md');
        if (fs.existsSync(goodvibesSkillFile)) {
            return {
                skillFile: goodvibesSkillFile,
                sourceType: 'goodvibes',
                skillPath: actualSkillName
            };
        }
    }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/goodvibes-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/goodvibes-codex` (add import at top)
...
Run: `node -c .codex/goodvibes-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/goodvibes-codex
...
**Files:**
- Modify: `.codex/goodvibes-codex` (lines 40-74)
...
Run: `.codex/goodvibes-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/goodvibes-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/goodvibes-codex` (lines 16-38, approximately)
...
Run: `.codex/goodvibes-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/goodvibes.js`
...
/**
 * Goodvibes plugin for OpenCode.ai
 *
...
const goodvibesSkillsDir = path.join(homeDir, '.config/opencode/goodvibes/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const GoodvibesPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/goodvibes.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/goodvibes.js
...
**Files:**
- Modify: `.opencode/plugin/goodvibes.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "goodvibes:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > goodvibes)
...
Run: `node -c .opencode/plugin/goodvibes.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the goodvibes and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const goodvibesSkills = skillsCore.findSkillsInDir(
            goodvibesSkillsDir,
            'goodvibes',
            3
          );
          ...
          const allSkills = [...personalSkills, ...goodvibesSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install goodvibes skills to ~/.config/opencode/goodvibes/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'goodvibes:';
...
```javascript
    'session.started': async () => {
      // Read using-goodvibes skill content
      const usingGoodvibesPath = skillsCore.resolveSkillPath(
        'using-goodvibes',
        goodvibesSkillsDir,
        personalSkillsDir
      );

      let usingGoodvibesContent = '';
      if (usingGoodvibesPath) {
        const fullContent = fs.readFileSync(usingGoodvibesPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingGoodvibesContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Goodvibes skills: \`goodvibes:skill-name\` (from ~/.config/opencode/goodvibes/skills/)
- Personal skills: \`skill-name\` (from ~/.config/opencode/skills/)
- Personal skills override goodvibes skills when names match
`;

      // Check for updates (non-blocking)
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/goodvibes')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/goodvibes && git pull` to update goodvibes.' :
        '';
...
**Below is the full content of your 'goodvibes:using-goodvibes' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingGoodvibesContent}
...
```markdown
# Installing Goodvibes for OpenCode
...
### 1. Install Goodvibes Skills

```bash
# Clone goodvibes skills to OpenCode config directory
mkdir -p ~/.config/opencode/goodvibes
git clone https://github.com/greglamb/goodvibes.git ~/.config/opencode/goodvibes
```

### 2. Install the Plugin

The plugin is included in the goodvibes repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/goodvibes/.opencode/plugin/goodvibes.js`
...
ln -s ~/.config/opencode/goodvibes/.opencode/plugin/goodvibes.js .opencode/plugin/goodvibes.js
```
...
```
use use_skill tool with skill_name: "goodvibes:brainstorming"
```
...
Personal skills override goodvibes skills with the same name.

## Updating

```bash
cd ~/.config/opencode/goodvibes
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/goodvibes/.opencode/plugin/goodvibes.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/goodvibes/skills`
...
- Report issues: https://github.com/greglamb/goodvibes/issues
- Documentation: https://github.com/greglamb/goodvibes
```
...
Goodvibes works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/goodvibes-codex use-skill goodvibes:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/goodvibes-codex bootstrap | head -30`
...
ls -l .opencode/plugin/goodvibes.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/goodvibes.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/goodvibes-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/goodvibes-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/goodvibes.js` created with tools and hooks
