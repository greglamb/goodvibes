...
Add full goodvibes support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port goodvibes to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/goodvibes-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/goodvibes.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/goodvibes/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/goodvibes-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "goodvibes:brainstorming")')
  }),
...
1. **Inject using-goodvibes content**
   - Full content of the using-goodvibes skill
...
```javascript
// .opencode/plugin/goodvibes.js
const skillsCore = require('../../lib/skills-core');
const path = require('path');
const fs = require('fs');
const { z } = require('zod');

export const GoodvibesPlugin = async ({ client, directory, $ }) => {
  const goodvibesDir = path.join(process.env.HOME, '.config/opencode/goodvibes');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingGoodvibes = await readSkill('using-goodvibes');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingGoodvibes}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── goodvibes-codex        # UPDATED: Use skills-core
│   ├── goodvibes-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── goodvibes.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/goodvibes-codex`
...
2. Update `.codex/goodvibes-codex` to use shared core
...
1. Create `.opencode/plugin/goodvibes.js`
